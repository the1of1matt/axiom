...
use crate::detect;
use crate::model::{
    ApplicationGraph, Component, Provider, Ownership, Readiness, ExecutionMode,
    VerificationResult, VerifyOutcome, ProjectClass,
};
use crate::util;
...
pub fn orchestrate(graph: &ApplicationGraph, verbose: bool) -> Result<RunningApp> {
    ...
    for id in &order {
        ...
        // Library / package / build-only: verify via cargo/npm build, do not treat as server
        if c.start.is_empty()
            || matches!(
                c.execution_mode,
        ...
        {
            match verify_non_server(c, verbose) {
                Ok(vr) => {
                    print_verification(&vr);
                    if vr.result == VerifyOutcome::Fail {
                        all_ready = false;
                    }
                }
                Err(e) => {
        ...
        for start_cmd in &c.start {
            ...
            match wait_ready(c, &mut mp, Duration::from_secs(45), verbose) {
                ...
                ReadyState::OneShotSuccess(code) => {
                    println!(
...
enum ReadyState {
    Ready,
    /// One-shot CLI finished with success exit code.
    OneShotSuccess(i32),
    StartedUnknown,
    Failed(String),
}
...
fn wait_ready(c: &Component, mp: &mut ManagedProcess, timeout: Duration, verbose: bool) -> ReadyState {
    ...
    let oneshot = matches!(
        c.execution_mode,
        ExecutionMode::OneShotCli | ExecutionMode::TestSuite
    );

    thread::sleep(Duration::from_millis(400));
    match mp.child.try_wait() {
        Ok(Some(status)) => {
            return interpret_exit(c, status, oneshot);
        }
        Ok(None) => {}
        Err(e) => return ReadyState::Failed(e.to_string()),
    }

    // Prefer HTTP/TCP checks on discovered ports (servers)
    if !c.ports.is_empty() && !oneshot {
        while Instant::now() < deadline {
            if let Ok(Some(status)) = mp.child.try_wait() {
                return interpret_exit(c, status, false);
            }
            for port in &c.ports {
                match check_port_ready(*port, verbose) {
                    PortCheck::Ready | PortCheck::OpenTcpOnly => {
                        if let Ok(Some(status)) = mp.child.try_wait() {
                            return interpret_exit(c, status, false);
                        }
    ...
    if oneshot {
        // Wait for CLI to finish (up to timeout)
        while Instant::now() < deadline {
            match mp.child.try_wait() {
                Ok(Some(status)) => return interpret_exit(c, status, true),
                Ok(None) => thread::sleep(Duration::from_millis(100)),
                Err(e) => return ReadyState::Failed(e.to_string()),
            }
        }
        // Still running after timeout — for CLI that's unusual; treat as unknown
        return ReadyState::StartedUnknown;
    }
    ...
    match mp.child.try_wait() {
        Ok(Some(status)) => interpret_exit(c, status, oneshot),
...
fn interpret_exit(
    ...
    if oneshot || matches!(c.execution_mode, ExecutionMode::OneShotCli | ExecutionMode::TestSuite)
    {
        if status.success() {
            return ReadyState::OneShotSuccess(code);
        }
        return ReadyState::Failed(format!(
            "command exited with {} (execution mode: {:?})",
            status, c.execution_mode
        ));
    }
    // Persistent server should not exit
    ReadyState::Failed(format!(
        "process exited with {} while a persistent process was expected ({:?})",
        status, c.execution_mode
    ))
}

fn verify_non_server(c: &Component, verbose: bool) -> Result<VerificationResult> {
    ...
    if c.start.is_empty() {
        ...
        if c.info.has_cargo_toml {
            ...
            let mut cmd = crate::platform::command("cargo");
            cmd.args(["check", "--message-format=short"]);
            cmd.current_dir(&c.info.path);
            if let Some(spec) = detect::rust_toolchain_spec(&c.info.path) {
                if detect::has_command("rustup") {
                    cmd = Command::new("rustup");
                    cmd.args(["run", &spec, "cargo", "check", "--message-format=short"]);
                    cmd.current_dir(&c.info.path);
                }
            }
            let status = cmd.status().context("cargo check")?;
            if status.success() {
                return Ok(VerificationResult {
                    project_type: format!("Rust {:?}", c.project_class),
                    environment_prepared: true,
                    dependencies_installed: true,
                    process_started: false,
                    listening_port: None,
                    health: "cargo check OK".into(),
            ...
            return Ok(VerificationResult {
                ...
                process_started: false,
                listening_port: None,
                health: format!("cargo check failed: {}", status),
                exit_code: status.code(),
                result: VerifyOutcome::Fail,
                notes,
            });
        }
        if c.info.has_pyproject || c.info.kinds.contains(&crate::detect::ProjectKind::Python) {
            notes.push(
                "Detected Python package without application entry point.".into(),
            );
            notes.push(
                "Axiom will not execute arbitrary .py files (e.g. under action/, tests/).".into(),
            );
            return Ok(VerificationResult {
                project_type: "Python package".into(),
                environment_prepared: true,
                dependencies_installed: true,
                process_started: false,
                listening_port: None,
        ...
        return Ok(VerificationResult {
            project_type: format!("{:?}", c.project_class),
            environment_prepared: true,
            dependencies_installed: true,
            process_started: false,
            listening_port: None,
            health: "nothing to run".into(),
            exit_code: None,
            result: VerifyOutcome::Skipped,
            notes,
        });
...
fn print_verification(vr: &VerificationResult) {
    ...
    println!("    project:     {}", vr.project_type);
    println!("    prepared:    {}", vr.environment_prepared);
    println!("    deps:        {}", vr.dependencies_installed);
    println!("    health:      {}", vr.health);
    if let Some(c) = vr.exit_code {
        println!("    exit code:   {}", c);
    }
    for n in &vr.notes {
        println!("    · {}", n);
    ...
    match vr.result {
        VerifyOutcome::Pass => println!("    result:      PASS"),
        VerifyOutcome::Fail => println!("    result:      FAIL"),
        VerifyOutcome::Partial => println!("    result:      PARTIAL"),
        VerifyOutcome::Skipped => println!("    result:      SKIPPED (not a runnable app)"),
    }
}
...
pub fn supervise(app: &mut RunningApp) -> Result<()> {
    // One-shots / libraries leave no children — exit cleanly
    if app.children.is_empty() {
        println!();
        println!("✓ No persistent processes to supervise — done.");
        return Ok(());
    }
    ...
    loop {
        ...
        app.children
            .retain_mut(|mp| matches!(mp.child.try_wait(), Ok(None)));
        if app.children.is_empty() {
            println!("✓ All supervised processes completed.");
            return Ok(());
        }
