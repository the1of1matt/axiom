...
fn expand_runtime_components(info: ProjectInfo) -> Vec<Component> {
    ...
    let py_resolved = detect::resolve_python_entry(&info.path);
    let py_has_app = py_resolved
        .as_ref()
        .map(|e| !matches!(e.kind, detect::PythonEntryKind::None))
        .unwrap_or(false);
    ...
    if has_python {
        ...
        if let Some(ref e) = py_resolved {
            py_info.markers.push(e.evidence.clone());
        }
        ...
        c.role = if matches!(c.execution_mode, model::ExecutionMode::Library) {
            ComponentRole::Library
        } else {
...
fn component_from_info(info: ProjectInfo) -> Component {
    ...
    let (execution_mode, project_class) = crate::detect::classify_target(&info, &start);
    Component {
        id: info.name.clone(),
        info,
...
fn start_commands(info: &ProjectInfo) -> Vec<String> {
    ...
    match info.primary_kind() {
        ...
        ProjectKind::Rust => {
            cmds.extend(crate::detect::rust_start_commands(info));
        }
        ProjectKind::Python | ProjectKind::Shell => {
            if let Some(entry) = crate::detect::resolve_python_entry(&info.path) {
                let py = crate::deps::python_interpreter(&info.path);
                match entry.kind {
                    crate::detect::PythonEntryKind::Module { ref module } => {
                        cmds.push(format!("{} -m {}", py, module));
                    }
                    crate::detect::PythonEntryKind::Script { ref rel } => {
                        cmds.push(format!("{} {}", py, rel));
                    }
                    crate::detect::PythonEntryKind::ConsoleScript { ref name } => {
                        // Prefer venv console script after prepare; fallback to -m
                        cmds.push(format!("{} -m {}", py, name));
                    }
                    crate::detect::PythonEntryKind::None => {}
                }
            }
        }
        ProjectKind::Go => {
